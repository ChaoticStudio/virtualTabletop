...
export class InitiativeComponent implements OnInit {
  ...
  addToTurnMember() {
    ...
    const id = this.list.hasChildNodes()
      ? ++this.turnMembers
      : (this.turnMembers = 1)
    const initNameValue = (<HTMLInputElement>(
    ...
    let initiative = (<HTMLInputElement>document.getElementById('initRoll'))
      .value
    if (initiative === '') {
      initiative = String(this.roll20())
    } else if (parseInt(initiative, 10) < 0 || initiative.startsWith('+')) {
      initiative = String(
        this.roll20(parseInt(initiative.replace('+', ''), 10))
      )
    }
    ...
    characterName.addEventListener('click', event =>
      this.updateName(event.target)
    )
    characterNameInput.addEventListener('blur', event =>
      this.updateName(event.target, true)
    )
    characterInitiative.appendChild(document.createTextNode(initiative))
    characterInitiative.addEventListener('click', event =>
      this.updateInitiative(event.target, false)
    )
    ...
    refreshButton.addEventListener('click', event => {
      this.updateInitiative(
        event.target.parentNode.parentNode.firstElementChild.children[2]
      )
    })
  ...
  updateInitiative(theOne2bUpdated, random = true) {
    if (random) {
      ...
      const initiativeInput = <HTMLInputElement>(
        initiative.parentElement.querySelector('.initItemInitiativeInput')
      )
      ...
      if (!this.hasEvent) {
        ...
        initiativeInput.addEventListener('blur', event => {
          ...
          const init = <HTMLDivElement>(
            input.parentElement.querySelector('.initItemValue')
          )
  ...
  updateName(theOne2bRenamed, isInput = false) {
    if (isInput) {
      ...
      const itemName = <HTMLDivElement>(
        itemInput.parentElement.querySelector('.initItemName')
      )
      ...
      const itemInput = <HTMLInputElement>(
        itemName.parentElement.querySelector('.initItemNameInput')
      )
