...
export class ThreejsComponent implements OnInit {
    ...
    document.querySelector('#scene')
      ...
      .addEventListener( 'click', event => {
        ...
        const bounds = document.querySelector('#scene').getBoundingClientRect();
				this.mouse.x =   ( ( event.x - bounds.left ) / ( bounds.right  - bounds.left ) ) * 2 - 1;
        this.mouse.y = - ( ( event.y - bounds.top )  / ( bounds.bottom - bounds.top) ) * 2 + 1;

        this.raycaster.setFromCamera( this.mouse, this._camera );
        let intersects = this.raycaster.intersectObjects( this._scene.children, true );
  ...
  private _loadObjects = (objectPath: string[]) => {
    ...
    for (let i = 0; i < objectPath.length; i++) {
      this._loader.load(objectPath[i], gltf => {
        ...
        this._diceObj[i].traverse(child => {
          if(child instanceof THREE.Mesh) {
            if (child.material["name"] === "Dice")
              ...
              child.material = new MeshToonMaterial({color: 0xff6100});
          }
        });
        this._diceObj[i].scale.set(1.5,1.5,1.5);
        this._diceObj[i].position.set(3.5 * Math.cos(i * (2*Math.PI/objectPath.length)), -.5, 3.5 * Math.sin( i * (2*Math.PI/objectPath.length)));
      ...
      }, xhr => checkProgress(xhr.loaded / xhr.total * 100 / objectPath.length))
    }
  }
